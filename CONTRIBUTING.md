...
- **
pm run dev**: Starts ay develop to hot-reload changes as you edit TypeScript/React source files.
- **
pm run lint**: Runs ay lint to validate package.json, extension icons, store metadata, ESLint rules, and Prettier formatting.
- **
pm run fix-lint**: Automatically fixes formatting and style issues using Prettier and ESLint.
- **
pm run build**: Compiles the extension bundle and runs the postbuild step to ensure compatibility.

### Verification Checklist Before Submitting a PR
- [ ] Code compiles cleanly with 
pm run build.
- [ ] 
pm run lint passes with 0 errors.
